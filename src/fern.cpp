...
namespace silva {
  ...
  namespace impl {
    parse_root_t fern_root{
        .rules =
            {
                parse_root_t::rule_t{.name = "Fern", .precedence = 0},
                parse_root_t::rule_t{.name = "LabeledItem", .precedence = 0},
                parse_root_t::rule_t{.name = "Label", .precedence = 0},
                parse_root_t::rule_t{.name = "Item", .precedence = 0},
                parse_root_t::rule_t{.name = "Item", .precedence = 1},
            },
    };
    ...
    struct fern_nursery_t : public parse_tree_nursery_t {
      ...
      fern_nursery_t(const tokenization_t* tokenization_)
        : parse_tree_nursery_t(tokenization_, &fern_root)
      {
      }
  ...
  // fern parse_tree visitation functions ////////////////////////////////////////
  ...
  labeled_item_t parse_tree_to_item(const parse_tree_t* pt, const index_t start_node)
  {
    ...
    SILVA_ASSERT(pt->visit_children(
