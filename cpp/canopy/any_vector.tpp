...
namespace silva::test {
  TEST_CASE("any_vector", "[any_vector_t]")
  {
    ...
    CHECK(av.apply(i1, to_string).as_string_view() == "Hello");
    CHECK(av.apply(i2, to_string).as_string_view() == "314");
    CHECK(av.apply(i3, to_string).as_string_view() == "[5 1 3]");
    CHECK(av.size() == 3);
    CHECK(av.byte_size() ==
    ...
    CHECK(av.size() == 3);
    CHECK(av.apply(i1, to_string).as_string_view() == "Hello");
    CHECK(av.apply(i4, to_string).as_string_view() == "[42 1.5]");
    CHECK(av.apply(i5, to_string).as_string_view() == "[123 2.5]");
    const auto i6 = av.push_back("World");
    CHECK(av.apply(i6, to_string).as_string_view() == "World");
  }
}
