...
TEST_CASE("operator-precedence", "")
{
  ...
  const auto op_prec_tt =
      share(SILVA_EXPECT_REQUIRE(tokenize(token_context_t::get(), "", op_prec_source_code)));
  ...
  const auto expr_tt =
      share(SILVA_EXPECT_REQUIRE(tokenize(token_context_t::get(), "", expr_source_code)));
