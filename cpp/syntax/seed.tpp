...
[0]Silva.Seed                                     - SimpleFern ... number ]
  ...
  const tree_node_index_t tni{.parse_tree = sfpr->seed_parse_trees.front().get()};
  CHECK(sfpr->rule_exprs.at(fni_sf) == tni.with_node_index(6));
  CHECK(sfpr->rule_exprs.at(fni_li) == tni.with_node_index(19));
  CHECK(sfpr->rule_exprs.at(fni_l) == tni.with_node_index(31));
  CHECK(sfpr->rule_exprs.at(fni_i) == tni.with_node_index(35));
  REQUIRE(sfpr->nonterminal_rules.size() == 4);
  CHECK(sfpr->nonterminal_rules.at(tni.with_node_index(12)) == fni_li);
  CHECK(sfpr->nonterminal_rules.at(tni.with_node_index(24)) == fni_l);
  CHECK(sfpr->nonterminal_rules.at(tni.with_node_index(27)) == fni_i);
  CHECK(sfpr->nonterminal_rules.at(tni.with_node_index(37)) == fni_sf);
