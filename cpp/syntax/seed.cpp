...
namespace silva {
  ...
  name_id_style_t seed_name_style(syntax_context_ptr_t tcp)
  {
  ...
  parse_axe::parse_axe_t create_parse_axe_expr(syntax_context_ptr_t tcp)
  {
  ...
  namespace impl {
    struct seed_parse_tree_nursery_t : public parse_tree_nursery_t {
      ...
      expected_t<parse_tree_node_t> nonterminal()
      {
        ...
        ss_rule.add_proto_node(SILVA_EXPECT_FWD(nonterminal_base()));
      ...
      expected_t<parse_tree_node_t> axe_ops()
      {
        ...
        ss_rule.add_proto_node(SILVA_EXPECT_FWD(axe_op_type()));
      ...
      expected_t<parse_tree_node_t> axe_level()
      {
        ...
        token_index += 1;
        ss_rule.add_proto_node(SILVA_EXPECT_FWD(axe_assoc()));
      ...
      expected_t<parse_tree_node_t> axe()
      {
        ...
        while (num_tokens_left() >= 1 && token_id_by() == tt_dash) {
          token_index += 1;
          ss_rule.add_proto_node(SILVA_EXPECT_FWD(axe_level()));
        }
      ...
      expected_t<parse_tree_node_t> expr_or_alias()
      {
        ...
        token_index += 1;
        ss_rule.add_proto_node(SILVA_EXPECT_FWD(expr()));
        return ss_rule.commit();
      }
      ...
      expected_t<parse_tree_node_t> seed()
      {
        ...
        while (num_tokens_left() >= 1 && token_id_by() == tt_dash) {
          token_index += 1;
          ss_rule.add_proto_node(SILVA_EXPECT_FWD(rule()));
        }
        return ss_rule.commit();
      }
    };
  ...
  unique_ptr_t<seed_engine_t> seed_seed_engine(syntax_context_ptr_t tcp)
  {
