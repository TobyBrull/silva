...
namespace silva::lox {
  ...
  expected_t<void> interpreter_t::resolve(parse_tree_span_t pts)
  {
    ...
    auto result = pts.visit_subtree([&](const span_t<const tree_branch_t> path,
      ...
      if (pts_curr.rule_name() == lexicon->ni_stmt_block) {
      ...
      else if (pts_curr.rule_name() == lexicon->ni_function_param) {
        if (is_on_entry(event)) {
        ...
        if (is_on_entry(event)) {
          ...
          const name_id_t parent_rule_name = (path.size() >= 2)
              ? pts.node_at(path[path.size() - 2].node_index).rule_name
              : name_id_t{};
  ...
  struct evaluation_t {
    ...
    expected_t<object_ref_t> call_function(object_ref_t callee, const parse_tree_span_t pts_args)
    {
      if (callee->holds_function()) {
        ...
        SILVA_EXPECT(arity == pts_args.num_children(),
                     MINOR,
                     "trying to call <function {}>, which has {} parameters, with {} arguments",
                     fun.pts,
                     arity,
                     pts_args.num_children());
        ...
        for (index_t i = 0; i < arity; ++i) {
          ...
          auto val = SILVA_EXPECT_FWD(expr_or_atom(*args_it),
                                      ...
                                      pts_args,
                                      i);
          const token_id_t ti_param = SILVA_EXPECT_FWD((*params_it).token());
...
#define BINARY(op_rule_name, op_func)                                                       \
  ...
  {                                                                                         \
    ...
    return op_func(intp->object_pool, std::move(lhs_res), std::move(rhs_res));              \
      ...
      const name_id_t rn = pts.rule_name();
      if (false) {
      }
      ...
      else if (rn == lexicon.ni_expr_member)
      {
        ...
        auto lhs_ref =
            SILVA_EXPECT_FWD(expr_or_atom(pts_lhs),
                             ...
                             pts);
        SILVA_EXPECT(pts_rhs.rule_name() == lexicon.ni_expr_atom,
                     MAJOR,
  ...
  struct execution_t {
    ...
    expected_t<return_t<object_ref_t>> decl(const parse_tree_span_t pts, scope_ptr_t& scope)
    {
      if (pts.rule_name() == lexicon.ni_decl_var) {
        ...
        if (pts_children.size == 2) {
          initializer = SILVA_EXPECT_FWD(intp->evaluate(pts_children[1], scope),
        ...
        while (it != end) {
          ...
          cc.methods[method_id] = intp->object_pool.make(std::move(ff));
          ++it;
        }
    ...
    expected_t<return_t<object_ref_t>> stmt(const parse_tree_span_t pts, scope_ptr_t& scope)
    {
      ...
      if (rule_name == lexicon.ni_stmt_print) {
        ...
        SILVA_EXPECT(it != end, MAJOR);
        if (cond->is_truthy()) {
          auto res = SILVA_EXPECT_FWD(intp->execute(*it, scope));
          if (res.has_value()) {
            return res;
          }
        }
        ...
        while (true) {
          ...
          auto res = SILVA_EXPECT_FWD(intp->execute(pts_body, scope));
          if (res.has_value()) {
            return res;
          }
          SILVA_EXPECT_FWD(intp->evaluate(pts_inc, scope),
                           "{} error evaluating for-condition",
                           pts);
        }
      }
