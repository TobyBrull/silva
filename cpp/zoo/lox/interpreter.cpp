...
namespace silva::lox {
  ...
  expected_t<void> interpreter_t::load_builtins(const parser_t& parser)
  {
    ...
    for (auto& [name, builtin]: builtins) {
      ...
      SILVA_EXPECT_ASSERT(scopes.root().define(name, std::move(builtin)));
    }
    return {};
  }
  ...
  expected_t<void> interpreter_t::resolve(parse_tree_span_t pts)
  {
    ...
    auto result = pts.visit_subtree([&](const span_t<const tree_branch_t> path,
      ...
      if (pts_curr[0].rule_name == lexicon->ni_stmt_block) {
        ...
        const auto ti = pts.ptp->tp->tokens[pts_curr[0].token_begin + 1].token_id;
        if (is_on_entry(event)) {
        ...
        // « Class ⊙ = "class" identifier Super ... »: the "identifier" twig now occupies node
        // index 1 (a single leaf), so the "Super" node is at index 2.
        const auto pts_super = pts_curr.sub_tree_span_at(2);
        ...
        if (pts_curr[0].token_begin < pts_curr[0].token_end) {
          ...
          const auto ti_sub   = pts.ptp->tp->tokens[p_node.token_begin + 1].token_id;
          const auto ti_super = pts.ptp->tp->tokens[pts_curr[0].token_begin + 1].token_id;
        ...
        if (is_on_entry(event)) {
          const auto ti = pts.ptp->tp->tokens[pts_curr[0].token_begin].token_id;
          static_scopes.back().variables.emplace(ti, true);
          static_scopes.push_back(static_scope_t{.class_type = static_scopes.back().class_type});
        }
        if (is_on_exit(event)) {
        ...
        if (is_on_entry(event)) {
          const auto& token = pts.ptp->tp->tokens[pts_curr[0].token_begin];
          ...
          const name_id_t pr =
              (path.size() >= 2) ? pts[path[path.size() - 2].node_index].rule_name : name_id_t{};
          const index_t parent_num_children =
              (path.size() >= 2) ? pts[path[path.size() - 2].node_index].num_children : 0;
          // The Expr-axe interleaves "oper" nodes among the operands, so the member-access
          // right-hand-side is identified as the parent's last child, and the call callee as its
          // first child.
          const bool is_member_access =
              (pr == lexicon->ni_expr_member && path.back().child_index == parent_num_children - 1);
          ...
          if ((is_identifier && !is_keyword && !is_member_access && !is_func_callee) || is_super) {
            ...
            if (const auto it = vars.find(token.token_id); it != vars.end()) {
              SILVA_EXPECT(it->second,
                           MINOR,
                           "{} can't read local variable in its own initializer",
                           pts_ti);
            }
            if (const auto dist_val = count_variable(pts_ti, token.token_id)) {
  ...
  struct evaluation_t {
    ...
    // The Expr-axe now emits "oper" nodes for the operator tokens, interspersed among the operands.
    // These carry no meaning for evaluation and are skipped here.
    template<index_t N>
    ...
    expected_t<object_ref_t> call_function(object_ref_t callee, const parse_tree_span_t pts_args)
    {
      if (callee->holds_function()) {
        const function_t& fun = std::get<function_t>(callee->data);
        ...
        return fb.impl(intp->object_pool, args);
      }
      else {
...
#define UNARY(op_rule_name, op_func)                                                       \
  ...
  {                                                                                        \
    const auto [node_idx] = SILVA_EXPECT_FWD(operand_children<1>(pts));                    \
...
#define BINARY(op_rule_name, op_func)                                                 \
  ...
  {                                                                                   \
    ...
    return op_func(intp->object_pool, std::move(lhs_res), std::move(rhs_res));        \
      ...
      UNARY(lexicon.ni_expr_u_exc, neg)
      UNARY(lexicon.ni_expr_u_sub, inv)
      BINARY(lexicon.ni_expr_b_mul, mul)
      BINARY(lexicon.ni_expr_b_div, div)
      BINARY(lexicon.ni_expr_b_add, add)
      BINARY(lexicon.ni_expr_b_sub, sub)
      BINARY(lexicon.ni_expr_b_lt, lt)
      BINARY(lexicon.ni_expr_b_gt, gt)
      BINARY(lexicon.ni_expr_b_lte, lte)
      BINARY(lexicon.ni_expr_b_gte, gte)
      BINARY(lexicon.ni_expr_b_eq, eq)
      ...
      else if (rn == lexicon.ni_expr_call)
      {
        ...
        if (callee->holds_function() || callee->holds_function_builtin()) {
          ...
          if (const auto it = cc.methods.find(lexicon.ti_init); it != cc.methods.end()) {
            object_ref_t init_fun_ref = SILVA_EXPECT_FWD(
                member_bind(retval, {}, lexicon.ti_init, intp->object_pool, lexicon.ti_this));
      ...
      else if (rn == lexicon.ni_expr_member)
      {
        ...
        SILVA_EXPECT(pts_rhs[0].rule_name == lexicon.ni_expr_atom,
                     MAJOR,
      ...
      else if (rn == lexicon.ni_expr_b_assign)
      {
        ...
        if (lhs_pts[0].rule_name == lexicon.ni_expr_member) {
          ...
          ci.fields[token.token_id] = rhs_ref;
        }
...
#undef UNARY
    ...
    expected_t<object_ref_t> atom(const parse_tree_span_t pts)
    {
      ...
      if (token.token_id == lexicon.ti_super) {
        ...
        const auto* super_ptr = scope.get_at(lexicon.ti_super, dist);
        ...
        const token_id_t field_name = pts.ptp->tp->tokens[pts[0].token_begin + 2].token_id;
        ...
        object_ref_t* ptr = [&] {
          ...
          if (it != intp->resolution.end()) {
            return scope.get_at(token.token_id, it->second);
          }
          else {
            return scope.get(token.token_id);
          }
        }();
        SILVA_EXPECT(ptr != nullptr,
                     MINOR,
                     "trying to resolve variable {}",
                     sfp->token_id_wrap(token.token_id));
        return *ptr;
      }
      else {
        SILVA_EXPECT(false,
  ...
  struct execution_t {
    ...
    // which are read via their tokens; skip them for structural navigation.
    array_t<index_t> structural_child_indexes(const parse_tree_span_t& pts) const
    {
      array_t<index_t> retval;
      for (const auto [node_index, child_index]: pts.children_range()) {
        const name_id_t rn = pts[node_index].rule_name;
        if (rn != lexicon.ni_identifier && rn != lexicon.ni_number && rn != lexicon.ni_string &&
            rn != lexicon.ni_expr_oper) {
    ...
    expected_t<return_t<object_ref_t>> decl(const parse_tree_span_t pts, scope_ptr_t& scope)
    {
      ...
      if (rule_name == lexicon.ni_decl_var) {
        const token_id_t var_name = pts.ptp->tp->tokens[pts[0].token_begin + 1].token_id;
        ...
        const token_id_t fun_name = pts.ptp->tp->tokens[pts[0].token_begin + 1].token_id;
        ...
        const token_id_t class_name = pts.ptp->tp->tokens[pts[0].token_begin + 1].token_id;
        ...
        if (pts_super[0].token_begin < pts_super[0].token_end) {
          const auto ti_super = pts.ptp->tp->tokens[pts_super[0].token_begin + 1].token_id;
          ...
          SILVA_EXPECT_FWD(used_scope.define(lexicon.ti_super, cc.superclass));
        }
        cc.pts = pts;
        ++ci;
