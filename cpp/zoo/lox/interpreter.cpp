  ...
  expected_t<void> interpreter_t::resolve(parse_tree_span_t pts)
  {
    ...
    auto result = pts.visit_subtree([&](const span_t<const tree_branch_t> path,
      ...
      if (pts_curr[0].rule_name == lexicon->ni_stmt_block) {
        ...
        const auto ti = SILVA_EXPECT_FWD(child_token(pts_curr));
        if (is_on_entry(event)) {
        ...
        const bool has_super = is_non_empty(pts_super);
        if (is_on_entry(event)) {
          const auto ti = SILVA_EXPECT_FWD(child_token(pts_curr));
        ...
        if (is_non_empty(pts_curr)) {
          const auto pts_sub  = pts.sub_tree_span_at(path[path.size() - 2].node_index);
          const auto ti_sub   = SILVA_EXPECT_FWD(child_token(pts_sub));
          const auto ti_super = SILVA_EXPECT_FWD(child_token(pts_curr));
        ...
        if (is_on_entry(event)) {
          const auto ti = SILVA_EXPECT_FWD(pts_curr.token());
          static_scopes.back().variables.emplace(ti, true);
        }
      }
      ...
      if (callee->holds_function()) {
        ...
        for (index_t i = 0; i < arity; ++i) {
          ...
          const token_id_t ti_param =
              SILVA_EXPECT_FWD(fun_params.sub_tree_span_at(params_it.pos).token());
...
#undef UNARY_PREFIX
    ...
    expected_t<object_ref_t> atom(const parse_tree_span_t pts)
    {
      ...
      if (token.token_id == lexicon.ti_super) {
        ...
        const token_id_t field_name = token.member_token_id;
  ...
  struct execution_t {
    ...
    expected_t<return_t<object_ref_t>> decl(const parse_tree_span_t pts, scope_ptr_t& scope)
    {
      ...
      if (rule_name == lexicon.ni_decl_var) {
        const token_id_t var_name = SILVA_EXPECT_FWD(child_token(pts));
        ...
        const token_id_t fun_name = SILVA_EXPECT_FWD(child_token(func_pts));
        ...
        const token_id_t class_name = SILVA_EXPECT_FWD(child_token(pts));
        ...
        if (is_non_empty(pts_super)) {
          const auto ti_super              = SILVA_EXPECT_FWD(child_token(pts_super));
        ...
        while (it != end) {
          ...
          const token_id_t method_name = SILVA_EXPECT_FWD(child_token(pts_method));
