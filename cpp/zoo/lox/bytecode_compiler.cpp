...
namespace silva::lox::bytecode {
  ...
  struct compile_run_t {
    ...
    compile_run_t(compiler_t* compiler, chunk_t& target)
      : compiler(compiler), nursery{.chunk = target}
    {
    }
    ...
    expected_t<void> expr(const parse_tree_span_t pts)
    {
      ...
      if (pts[0].rule_name == lexicon.ni_expr_atom) {
        ...
        SILVA_EXPECT_FWD(nursery.backpatch_index_instr(j1, bytecode.size() - j1));
      }
        ...
        SILVA_EXPECT_FWD(nursery.append_simple_instr(pts, POP));
        SILVA_EXPECT_FWD(expr(pts.sub_tree_span_at(rhs)));
        SILVA_EXPECT_FWD(nursery.backpatch_index_instr(j2, bytecode.size() - j2));
      }
        ...
        SILVA_EXPECT_FWD(nursery.append_index_instr(pts, CALL, pts_args[0].num_children));
      }
    ...
    expected_t<void> decl(const parse_tree_span_t pts)
    {
      ...
      if (pts[0].rule_name == lexicon.ni_decl_var) {
        ...
        chunk_nursery_t sub_nursery{*chunk};
        SILVA_EXPECT_FWD(sub_nursery.append_simple_instr(func_pts, NIL));
      ...
      if (compiler->scope_depth == 0) {
        SILVA_EXPECT_FWD(nursery.append_index_instr(pts, DEFINE_GLOBAL, decl_name));
      }
      else {
        locals.push_back(compiler_t::local_t{
            .var_name    = decl_name,
    ...
    expected_t<void> stmt(const parse_tree_span_t pts)
    {
      ...
      if (rule_name == lexicon.ni_stmt_print) {
        ...
        const index_t cond_label = bytecode.size();
        ...
        const index_t inc_label = bytecode.size();
        SILVA_EXPECT_FWD(expr(pts.sub_tree_span_at(inc_idx)),
                         "{} error compiling 'for' increment",
                         pts);
        SILVA_EXPECT_FWD(nursery.append_simple_instr(pts, POP));
        SILVA_EXPECT_FWD(nursery.append_index_instr(pts, JUMP, cond_label - bytecode.size()));

        SILVA_EXPECT_FWD(nursery.backpatch_index_instr(j2, bytecode.size() - j2));
        const index_t body_label = bytecode.size();
        SILVA_EXPECT_FWD(go(pts.sub_tree_span_at(body_idx)), "{} error compiling 'for' body", pts);
        SILVA_EXPECT_FWD(nursery.append_index_instr(pts, JUMP, inc_label - bytecode.size()));
        ...
        SILVA_EXPECT_FWD(nursery.append_simple_instr(pts, RETURN));
      }
        ...
        while (!locals.empty() && locals.back().scope_depth == compiler->scope_depth) {
          locals.pop_back();
          SILVA_EXPECT_FWD(nursery.append_simple_instr(pts, POP));
        }
        compiler->scope_depth -= 1;
  ...
  expected_t<void> compiler_t::compile(const parse_tree_span_t pts, chunk_t& target)
  {
    compile_run_t run{this, target};
